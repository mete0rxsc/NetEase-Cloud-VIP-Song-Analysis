...
API_KEY = "lHY1oFqNOseVmtH1tk6GNVbNh8"
LEVEL = "jymaster"
...
class NetEaseMusicParser:
    ...
    def create_ui(self):
        ...
        # 使用Treeview显示结果
        self.result_tree = ttk.Treeview(result_frame, columns=("display", "full"), show="tree")
        ...
        # 歌词显示区域
        self.lyric_label = Label(self.root, text="", font=("微软雅黑", 14), anchor='center', fg="blue")
        self.lyric_label.pack(side=BOTTOM, fill=X, pady=10)

        # 时间显示标签
        self.time_label = Label(self.root, text="00:00 / 00:00", bd=1, relief=SUNKEN, anchor=E)
            ...
            if isinstance(value, dict):
                ...
                for i, item in enumerate(value):
                    if isinstance(item, dict):
                        ...
                        self.result_tree.insert(node, "end", text=f"项目 {i+1}", values=(display_value, full_value))
            else:
                ...
                if key.lower() == "lyric":
                    ...
                    if len(lines) > 0:
                        display_value = lines[0] + ("..." if len(lines) > 1 else "")

                # URL 缩略处理
                elif key.lower() == "url" and len(display_value) > 50:
                    display_value = display_value[:50] + "..."

                self.result_tree.insert(parent, "end", text=key, values=(display_value, full_value))
    ...
    def play_song(self):
        ...
        if not os.path.exists(save_path):
            choice = messagebox.askyesnocancel("选择播放方式", "歌曲尚未下载。\n请选择：\n\n是：在线播放（需要网络）\n否：下载后再播放\n取消：取消播放")
            if choice is None:
                return
            elif choice:
    ...
    def stream_and_play(self, url):
        ...
        threading.Thread(target=self._stream_audio, args=(url,), daemon=True).start()
    ...
    def _stream_audio(self, url):
        try:
            ...
            self.root.after(0, lambda: messagebox.showerror("错误", f"在线播放失败: {str(e)}"))
