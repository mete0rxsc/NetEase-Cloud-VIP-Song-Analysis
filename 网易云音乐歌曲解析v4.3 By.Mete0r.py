...
class NetEaseMusicParser:
    ...
    def open_settings(self):
        ...
        volume_scale = ttk.Scale(
            ...
            orient=HORIZONTAL,
            style="Rounded.Horizontal.TScale",
            command=lambda val: self._update_volume_label(val)  # 添加 command 回调
        )
    ...
    def _save_settings(self, dialog, new_key_entry, volume_scale):
        ...
        # 解除回调绑定，防止后续回调操作已销毁的控件
        volume_scale.configure(command=None)
    ...
    def _update_volume_label(self, val):
        if hasattr(self, 'settings_volume_value') and self.settings_volume_value.winfo_exists():
            self.settings_volume_value.config(text=f"{int(float(val))}")
